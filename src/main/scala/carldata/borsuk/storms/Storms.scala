...
object Storms {
  ...
  def getAllStorms(resolution: Duration, rainfall: TimeSeries[Double]): List[(String, StormParams)] = {
    ...
    val mergedSession = mergeSessions(baseSessions, baseSessions, listOfSessionWindows, resolution)
    mergedSession
  }
